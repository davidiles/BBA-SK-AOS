...
xval_PC_CL_complete <- data.frame()
if (file.exists("../output/xval_PC_CL_complete.RData")){
  load("../output/xval_PC_CL_complete.RData")
}

nruns <- xval_PC_CL_complete %>%
  group_by(Species) %>%
  summarize(nfolds = n())
...
species_to_fit = subset(nruns, nfolds <= 2)
for (fold in 5:1){
 for (sp_code in species_to_fit$Species){
    ...
    if (nrow(xval_PC_CL_complete)>0){
      if (nrow(subset(xval_PC_CL_complete,Species == sp_code & xval_fold == fold))>0) next
    }
  ...
  ',
    ...
    xval_PC_CL_complete <- rbind(xval_PC_CL_complete,
    ...
    save(xval_PC_CL_complete, file = "../output/xval_PC_CL_complete.RData")
