...
xval_PC_CL_complete2 <- data.frame()
if (file.exists("../output/xval_PC_CL_complete2.RData")){
  load("../output/xval_PC_CL_complete2.RData")
}
...
set.seed(999)
species_to_fit <- subset(species_distribution_summary, n_sq >= 50 & n_PC >= 50 & n_CL >= 50) %>%
  arrange(n_PC) %>%
  sample_n(50) %>%
  arrange(n_PC)

# Use same species list as before
load(file = "../output/xval_PC_CL_subset.RData")
species_to_fit <- unique(xval_PC_CL_subset$Species)
...
for (fold in 1:5){
 for (sp_code in species_to_fit){
    ...
    if (nrow(xval_PC_CL_complete2)>0){
      if (nrow(subset(xval_PC_CL_complete2,Species == sp_code & xval_fold == fold))>0) next
    }
    ...
    # -----------------------------
    # FIT MODEL TO 50% of POINT COUNTS
    # -----------------------------
  ...
  ',
    ...
    start <- Sys.time()
    fit_PC_50 <- bru(components = model_components,
    ...
    runtime_PC_50 <- difftime( end,start, units="mins")
    
    # -----------------------------
    # FIT MODEL TO 100% of POINT COUNTS
    # -----------------------------
  ...
  ',
    ...
    start <- Sys.time()
    fit_PC_100 <- bru(components = model_components,
    ...
    runtime_PC_100 <- difftime( end,start, units="mins")
    
    # -----------------------------
    # FIT MODEL TO 50% of PC + overlapping checklists
    # -----------------------------
  ...
  LT_distance(main = LT_duration,model = LT_duration_spde) +  
  ',
    ...
    fit_integrated_50_50 <- bru(components = model_components,
                     ...
                     like(family = "binomial",
                          ...
                          data = LT_sp[LT_sp$sq_id %!in% squares_to_withhold,]),
    ...
    # -----------------------------
    # FIT MODEL TO 50% of PC + 100% of checklists
    # -----------------------------
  ...
  ',
    ...
    # -----------------------------
    # FIT MODEL TO 100% of PC + 100% of checklists
    # -----------------------------
  ...
  ',
    ...
    start <- Sys.time()
    fit_integrated_100_100 <- bru(components = model_components,
    ...
    runtime_integrated_100_100 <- difftime( end,start, units="mins")
    ...
    pred_PC_50 <- generate(fit_PC_50, 
                               PC_xval, 
                               formula = pred_formula_PC,
                               n.samples = nsamp)
    
    pred_PC_100 <- generate(fit_PC_100, 
                           PC_xval, 
                           formula = pred_formula_PC,
                           n.samples = nsamp)
    ...
    pred_integrated_100_100 <- generate(fit_integrated_100_100, 
                            PC_xval, 
                            formula = pred_formula_PC,
                            n.samples = nsamp)
    
    # Add lognormal variance corrections, and exponentiate to place on count scale
    pred_PC_50     <- exp(pred_PC_50     + 0.5/summary(fit_PC_50)$inla$hyperpar["Precision for kappa_surveyID",4]     + 0.5/summary(fit_PC_50)$inla$hyperpar["Precision for kappa_squareID",4]     + 0.5/summary(fit_PC_50)$inla$hyperpar["Precision for kappa_squareday",4])
    pred_PC_100     <- exp(pred_PC_100     + 0.5/summary(fit_PC_100)$inla$hyperpar["Precision for kappa_surveyID",4]     + 0.5/summary(fit_PC_100)$inla$hyperpar["Precision for kappa_squareID",4]     + 0.5/summary(fit_PC_100)$inla$hyperpar["Precision for kappa_squareday",4])
    ...
    pred_integrated_100_100     <- exp(pred_integrated_100_100     + 0.5/summary(fit_integrated_100_100)$inla$hyperpar["Precision for kappa_surveyID",4]     + 0.5/summary(fit_integrated_100_100)$inla$hyperpar["Precision for kappa_squareID",4]     + 0.5/summary(fit_integrated_100_100)$inla$hyperpar["Precision for kappa_squareday",4])
    
    # &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    # Calculate crossvalidation scores
    # &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    
    cor_PC_50 <- AUC_PC_50 <- MSE_PC_50 <- lppd_PC_50 <- rep(NA,nsamp)
    cor_PC_100 <- AUC_PC_100 <- MSE_PC_100 <- lppd_PC_100 <- rep(NA,nsamp)
    cor_integrated_50_50 <- AUC_integrated_50_50 <- MSE_integrated_50_50 <- lppd_integrated_50_50 <- rep(NA,nsamp)
    cor_integrated_50_100 <- AUC_integrated_50_100 <- MSE_integrated_50_100 <- lppd_integrated_50_100 <- rep(NA,nsamp)
    cor_integrated_100_100 <- AUC_integrated_100_100 <- MSE_integrated_100_100 <- lppd_integrated_100_100 <- rep(NA,nsamp)
    
      for (i in 1:nsamp){
        cor_PC_50[i] <- as.numeric(cor(PC_xval$count, pred_PC_50[,i]))
        MSE_PC_50[i] <- mean((PC_xval$count - pred_PC_50[,i])^2)
        lppd_PC_50 <- log(dpois(PC_xval$count,pred_PC_50[,i]))
        lppd_PC_50[lppd_PC_50 == -Inf] = -1000
        lppd_PC_50[i] <- sum(lppd_PC_50)
        if (length(unique(PC_xval$presence))>1){
          suppressMessages({
            
            AUC_PC_50[i] <- as.numeric(auc(PC_xval$presence, 1-exp(-pred_PC_50[,i])))
            
          })
        }
        
        cor_PC_100[i] <- as.numeric(cor(PC_xval$count, pred_PC_100[,i]))
        MSE_PC_100[i] <- mean((PC_xval$count - pred_PC_100[,i])^2)
        lppd_PC_100 <- log(dpois(PC_xval$count,pred_PC_100[,i]))
        lppd_PC_100[lppd_PC_100 == -Inf] = -1000
        lppd_PC_100[i] <- sum(lppd_PC_100)
        if (length(unique(PC_xval$presence))>1){
          suppressMessages({
            
            AUC_PC_100[i] <- as.numeric(auc(PC_xval$presence, 1-exp(-pred_PC_100[,i])))
        ...
        cor_integrated_100_100[i] <- as.numeric(cor(PC_xval$count, pred_integrated_100_100[,i]))
        MSE_integrated_100_100[i] <- mean((PC_xval$count - pred_integrated_100_100[,i])^2)
        lppd_integrated_100_100 <- log(dpois(PC_xval$count,pred_integrated_100_100[,i]))
        lppd_integrated_100_100[lppd_integrated_100_100 == -Inf] = -1000
        lppd_integrated_100_100[i] <- sum(lppd_integrated_100_100)
        if (length(unique(PC_xval$presence))>1){
          suppressMessages({
            
            AUC_integrated_100_100[i] <- as.numeric(auc(PC_xval$presence, 1-exp(-pred_integrated_100_100[,i])))
    ...
    xval_PC_CL_complete2 <- rbind(xval_PC_CL_complete2,
                               data.frame(Species = sp_code,
                                          ...
                                          # Number of squares in which species was detected in full LT dataset
                                          n_sq_det_LT = n_sq_det_LT$n_sq_det,
                                          # Number of detections in full LT dataset
                                          n_det_LT = n_det_LT$n_det,
                                          
                                          # Number of detections in crossvalidation squares
                                          n_det_PC_xval = sum(PC_xval$count>0),
                                          n_det_SC_xval = sum(SC_xval$count>0),
                                          n_det_LT_xval = sum(LT_xval$count>0),
                                          
                                          # -------------------------------
                                          # Crossvalidation metrics
                                          # -------------------------------
                                          
                                          cor_PC_50  = mean(cor_PC_50),
                                          AUC_PC_50  = mean(AUC_PC_50),
                                          MSE_PC_50  = mean(MSE_PC_50),
                                          lppd_PC_50 = mean(lppd_PC_50),
                                          
                                          cor_PC_100  = mean(cor_PC_100),
                                          AUC_PC_100  = mean(AUC_PC_100),
                                          MSE_PC_100  = mean(MSE_PC_100),
                                          lppd_PC_100 = mean(lppd_PC_100),
                                          ...
                                          cor_integrated_100_100  = mean(cor_integrated_100_100),
                                          AUC_integrated_100_100  = mean(AUC_integrated_100_100),
                                          MSE_integrated_100_100  = mean(MSE_integrated_100_100),
                                          lppd_integrated_100_100 = mean(lppd_integrated_100_100)
                                          
                               )
    )
    
    save(xval_PC_CL_complete2, file = "../output/xval_PC_CL_complete2.RData")
    
    rm(list = c("fit_PC","pred_PC",
                "fit_PC_SC","pred_PC_SC",
                "fit_PC_LT","pred_PC_LT",
                "fit_PC_SC_LT","pred_PC_SC_LT"))
