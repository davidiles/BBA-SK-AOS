  ...
  ',
    ...
    suppressMessages(
      for (i in 1:length(AUC_PConly)){
        AUC_PConly[i] <- as.numeric(auc(PC_xval$presence, 1-exp(-pred_PConly[,i])))
        AUC_integrated[i] <- as.numeric(auc(PC_xval$presence, 1-exp(-pred_integrated[,i])))
      }
    )
    ...
    for (i in 1:length(cor_PConly)){
      cor_PConly[i] <- as.numeric(cor(PC_xval$count, pred_PConly[,i]))
      cor_integrated[i] <- as.numeric(cor(PC_xval$count, pred_integrated[,i]))
    }
    
    # correlation (for count predictions) at a 10 km square level
    cor_PConly_sq <- cor_integrated_sq <- rep(NA, ncol(pred_PConly))
    for (i in 1:length(cor_PConly)){
      tmp <- PC_xval %>% 
        as.data.frame() %>%
        mutate(pred_PConly = pred_PConly[,i],
               pred_integrated = pred_integrated[,i]) %>%
        group_by(sq_id) %>%
        summarize(sum_count = sum(count),
                  sum_pred_PConly = sum(pred_PConly),
                  sum_pred_integrated = sum(pred_integrated))
      
      cor_PConly_sq[i] <- as.numeric(cor(tmp$sum_count, tmp$sum_pred_PConly))
      cor_integrated_sq[i] <- as.numeric(cor(tmp$sum_count, tmp$sum_pred_integrated))
    ...
    for (i in 1:length(MSE_PConly)){
      MSE_PConly[i] <- mean((PC_xval$count - pred_PConly[,i])^2)
      MSE_integrated[i] <- mean((PC_xval$count - pred_integrated[,i])^2)
    }
    
    # MSE (for count predictions)
    MAE_PConly <- MAE_integrated <- rep(NA, ncol(pred_PConly))
    for (i in 1:length(MSE_PConly)){
      MAE_PConly[i] <- mean(abs(PC_xval$count - pred_PConly[,i]))
      MAE_integrated[i] <- mean(abs(PC_xval$count - pred_integrated[,i]))
    }
    ...
    xval_df_50km_TSS <- rbind(xval_df_50km_TSS,
                     data.frame(Species = sp_code,
                                ...
                                cor_PConly_sq = mean(cor_PConly),
                                cor_integrated_sq = mean(cor_integrated),
                                ...
                                # Mean absolute error
                                MAE_PConly = mean(MAE_PConly),
                                MAE_integrated = mean(MAE_integrated)
