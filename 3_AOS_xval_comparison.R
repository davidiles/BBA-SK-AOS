...
xval_df_50km <- data.frame()
if (file.exists("../AOS_precision/output/xval_df_50km.RData")){
...
for (sp_code in c("TEWA","RCKI","YRWA","SPSA")){
for (fold in sort(unique(SaskSquares$fold))){
  #for (sp_code in rev(species_to_fit$Species)){
    ...
    if (nrow(xval_df_50km)>0){
      if (nrow(subset(xval_df_50km,Species == sp_code & xval_fold == fold))>0) next
    }
  ...
  ',
    ...
    xval_df_50km <- rbind(xval_df_50km,
    ...
    save(xval_df_50km, file = "../AOS_precision/output/xval_df_50km.RData")
